...
class TrainStepsReduceLROnPlateau(TrainSteps):
    def __init__(self, training_batch_size, eval_batch_size, epoches, gradient_adjust_fn=lambda x: x):
        super(TrainStepsReduceLROnPlateau, self).__init__(training_batch_size, eval_batch_size, epoches)
    ...
    def call(self, training_datasets_fn, models_fn, losses_fn, optimizers_fn, log_dir, eval_datasets_fn=None,
        ...
        # propose optimizer_fn is SGD and learning_rate_fn is constant
        assert optimizers_fn._name == 'SGD'
        assert type(learning_rates_fn) == 'float'
